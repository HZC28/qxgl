...
import {login} from '../api/self/user'
import menusData from '../utils/menu.js'
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          // }, 1000)
          login({name:this.ruleForm.username,password:this.ruleForm.password}).then(res=>{
            if(res.data.code==100){
              this.$message.error(res.data.msg)
              return
            }
            let result=[]
            let menuarr=res.data.data.menus
            let childrenMenu1=[]
            let childrenMenu2=[]
            menuarr.forEach(val=>{
              for(let i=0;i<menusData.length;i++){
                if(menusData[i].menuname==val.menuname){
                  // JSON.stringify(JSON.parse(menusData[i]))
                  let a=JSON.parse(JSON.stringify(menusData[i]))
                  a.menus=[]
                  val.children.forEach(value=>{
                    for(let k=0;k<menusData[i].menus.length;k++){
                      if(menusData[i].menus[k].menuname==value.menuname){
                        a.menus.push(menusData[i].menus[k]);
                        break
                      }
                    }
                  })
